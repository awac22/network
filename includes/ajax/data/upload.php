        ...
        case 'photos':
            ...
            if ($_POST['handle'] == 'picture-user' || $_POST['handle'] == 'picture-page' || $_POST['handle'] == 'picture-group') {
             ...
             if ($result->num_rows > 0) 
			 {
				while($row = $result->fetch_assoc()) 
				{
					$id 	= $row['id']; 
					$name   = $row['name']; 
					$post_desc = "Network uploaded by  $name";
					$post_title = "Network image uploaded by  $name";
					$post_slug  = uniqid(); 
				}
			}
            ...
            if ($_POST["multiple"] == "true") {
                ...
                foreach ($files as $file) {
					...
					$path_art = 'https://anywhereanycity.com/network/content/uploads/'.$file_name;
					
					$created_at = date('Y-m-d h:i:s');
					$sql = "INSERT INTO posts(id,user_id,post_live,post_color,post_desc,post_title,post_slug,post_media,created_at)
							VALUES (NULL, $id ,1, 'bg-primary', '$post_desc', '$post_title', '$post_slug', '$path_art','$created_at')";
			        $conn->query($sql);  
                    $image_blured = 0;
			...
			move_uploaded_file($tmp_name, "$uploads_dir/$name");
             ...
             if ($result->num_rows > 0) 
			 {
				while($row = $result->fetch_assoc()) 
				{
					$id 	= $row['id']; 
					$name   = $row['name']; 
					$title = rand(10,100)."_".str_replace($extension,'',$_FILES['file']['name']);
					$size = $_FILES["file"]["size"];
					$post_slug  = uniqid(); 
					$created_at = date('Y-m-d h:i:s');
					$sql = "INSERT INTO songs(id,title,cover,source,source_format,uploaded_by,user_id,artist_id,public,created_at)
							VALUES (NULL, '$title' ,'','','file', 'user',$id,0,1,'$created_at')";
			        $conn->query($sql); 
					$last_id = mysqli_insert_id($conn);
					
					$uuid = gen_uuid();
					$sql = "INSERT INTO `media` (`id`, `model_type`, `model_id`, `uuid`, `collection_name`, `name`, `file_name`, `mime_type`, `disk`, `conversions_disk`, `size`, `manipulations`, `custom_properties`, `responsive_images`, `order_column`, `created_at`, `updated_at`) 
					VALUES (NULL, 'App\\\Song', '$last_id', '$uuid', 'cover', 'song_cover', 'song_cover.png', 'image/png', 'public', 'public', '11812', '[]', '[]', '[]', '38', '$created_at', '$created_at')";
					$conn->query($sql);
					$dir_last_id1 = mysqli_insert_id($conn);
                    $uuid = gen_uuid();
                    $mime_type=mime_content_type($_FILES['file']['tmp_name']);
					$sql = "INSERT INTO `media` (`id`, `model_type`, `model_id`, `uuid`, `collection_name`, `name`, `file_name`, `mime_type`, `disk`, `conversions_disk`, `size`, `manipulations`, `custom_properties`, `responsive_images`, `order_column`, `created_at`, `updated_at`) VALUES 
					(NULL, 'App\\\Song', '$last_id', '$uuid', '$extension', 'Z7soZU', '$file_name', '$mime_type', 'public', 'public', '$size', '[]', '{\"bitrate\":192}', '[]', '38', '$created_at', '$created_at')";  					
                   	$conn->query($sql);
					$dir_last_id = mysqli_insert_id($conn);
					$Dir= $_SERVER['DOCUMENT_ROOT']."/music/public/storage/".$dir_last_id;
					$Dir2= $_SERVER['DOCUMENT_ROOT']."/music/public/storage/".$dir_last_id1;
					$newpath = $_SERVER['DOCUMENT_ROOT']."/music/public/storage/".$dir_last_id."/".$file_name;;
					mkdir($Dir, 0777, true);
            ...
            if ($system['s3_enabled']) {
                ...
                if (!@move_uploaded_file($_FILES['file']['tmp_name'], $path)) {
                    modal("ERROR", __("Upload Error"), __("Sorry, can not upload the file"));
					  ...
					  copy($path,$newpath);
					  $path1    = $_SERVER['DOCUMENT_ROOT'].'/music/public/storage/35/song_cover.png';
					  $newpath2 = $_SERVER['DOCUMENT_ROOT'].'/music/public/storage/'.$dir_last_id1.'/song_cover.png';
					  copy($path1,$newpath2);
